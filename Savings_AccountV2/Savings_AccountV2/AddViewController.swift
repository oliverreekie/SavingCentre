...
class AddViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource{
    ...
    @IBAction func AddButton(_ sender: Any) {
        ...
        if let amount = Foundation.UserDefaults.standard.string(forKey: "amount") {
            print("Working")
            if let getAmount = insertTextField.text{
                let oldAmount = Int(amount)
                let newAmount = Int(getAmount)! + oldAmount!
            ...
            let getAmount = insertTextField.text
            let newAmount = Int(getAmount!)!
            Foundation.UserDefaults.standard.set(newAmount, forKey: "amount")
        }
        addingToCurrentAmount()
        insertTextField.text = ""
    }
        else{
            let alert = UIAlertController(title: "Amount is empty", message: "The amount to add has been left blank, please enter an amount before continuing", preferredStyle: .alert)
